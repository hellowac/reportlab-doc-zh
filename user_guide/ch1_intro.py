...
bullet(
    """defaultImageCaching: set to zero to inhibit the creation of .a85 files 
    on your
hard-drive. The default is to create these preprocessed PDF compatible image 
files for faster loading"""
)
bullet(
    """T1SearchPath: this is a python list of strings representing 
    directories that
...
disc(
    """For the full list of variables see the file 
    $reportlab/rl_settings.py$."""
)
heading3("Other modifications")
disc(
    """More complex modifications to the reportlab toolkit environment may be 
    made using one
of the modules $rep[ortlab.local_rl_mods$ (.py script in reportlab folder),
$reportlab_mods$ (.py file on the python path) or $~/.reportlab_mods$ (note 
no .py)."""
)
...
disc(
    """ReportLab 3.0 has been produced to help in the migration to Python 
    3.x.  Python 3.x will
be standard in future Ubuntu releases and is gaining popularity, and a good 
proportion
of major Python packages now run on Python 3.  """
)

bullet(
    """Python 3.x compatibility.  A single line of code should run on 2.7 and 
    3.6"""
)
bullet(""" __init__.py restricts to 2.7 or >=3.6""")
bullet(
    """__init__.py allow the import of on optional reportlab.local_rl_mods to 
    allow monkey patching etc."""
)
bullet(
    """rl_config now imports rl_settings, optionally local_rl_settings, 
    reportlab_settings.py & finally ~/.reportlab_settings"""
)
bullet(
    """ReportLab C extensions now live inside reportlab; _rl_accel is no 
    longer required. All _rl_accel imports now pass through 
    reportlab.lib.rl_accel"""
)
bullet(
    """xmllib is gone, alongside the paraparser stuff that caused issues in 
    favour of HTMLParser."""
)
...
bullet(
    """Removed reportlab/lib/ para.py & pycanvas.py.  These would better 
    belong in third party packages, which can make use of the monkeypatching 
    feature above."""
)
bullet(
