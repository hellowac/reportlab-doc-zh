...
cn_disc('$TableStyle$ 跨单元格命令')

disc('Our $Table$ classes support the concept of spanning, but it isn\'t '
...
cn_disc('表示列 $sc$ - $ec$ 和行 $sr$ - $er$ 中的单元格应该合并成一个超级单元格，'
        '其内容由单元格 $(sc, sr)$ 决定。'
...
disc('notice that we don\'t need to be conservative with our $GRID$ command. '
...
cn_disc("在任何样式命令中，第一行索引可以设置为特殊字符串 $'splitlast'$ 或 $'splitfirst'$ 中的一个，"
