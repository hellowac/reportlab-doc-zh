...
cn_disc(
    '该包提供了几个<i>Renderers</i> (渲染器)，它们知道如何将图纸绘制成不同的格式。'
...
cn_disc(
    '<i><font color=red>注意</font>：$pprint$ 是标准的Python库模块，'
...
cn_disc('这三种方法在这里似乎并没有什么作用，' '但正如我们将看到的那样，' '它们使我们的 $widgets$ 框架在处理非原始对象时更加强大。')
